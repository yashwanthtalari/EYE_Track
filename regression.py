...
class GazeRegressor:
    """
    Handles training, saving, loading, and predicting gaze screen coordinates
    using Ridge Regression with a 2nd-degree Polynomial Feature expansion.
    Loads rich multi-eye and 3D head position data from the SQLite database.
    """
    def __init__(self, models_dir="models", filename="regression.pkl", db_dir="data", db_filename="calibration.db"):
    ...
    def train_from_db(self):
        ...
        separate models for X and Y coordinates using a rich 10-feature vector.
        """
        records, columns = self.db.load_latest_session()
        if not records:
        ...
        # 10 features for maximum accuracy:
        # Left Eye (2), Right Eye (2), Head Rotation (3), Head Translation (3)
        feature_cols = [
    ...
    def predict(self, left_x, left_y, right_x, right_y, yaw, pitch, roll, tx, ty, tz):
        """
        Predicts screen coordinate (screen_x, screen_y) using the 10-feature vector.
        """
